...
export function StepPayment({ data, pathway: pathwayProp, onNext, onBack }: StepPaymentProps) {
  ...
  const amount = pathway === "PLUS" ? "£7,500" : "£2,500"
  const isTestMode = process.env.NODE_ENV !== "production"

  useEffect(() => {
    if (searchParams.get("cancelled") === "true") {
      setCancelled(true)
      router.replace("/apply?step=9")
    }
  }, [searchParams, router])
  ...
  const completeApplication = async (reference: string, provider: string) => {
    await fetch("/api/apply", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ step: 10, paymentReference: reference, paymentAmount: amount, paymentProvider: provider }),
    })
  }

  return (
    <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm sm:p-8">
      <h2 className="text-xl font-bold text-gray-900">Authorise One-Off Payment</h2>
      <p className="mt-1 text-sm text-gray-500">Section 9 of the Application / Programme Terms Form</p>
      ...
      <div className="mt-6 rounded-xl border border-gray-200 bg-gray-50/50 p-5">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-gray-500">One-off Programme Fee</p>
          </div>
            ...
            <PayPalButtons
              ...
              createOrder={async () => {
                ...
                try {
                  const res = await fetch("/api/onboarding/payment", {
                    method: "POST",
                    headers: { "Content-Type": "application/json" },
                    body: JSON.stringify({ provider: "PAYPAL", pathway, source: "apply" }),
                  })
              ...
              onApprove={async (details) => {
                ...
                try {
                  ...
                  await completeApplication(details.orderID, "PAYPAL")
      ...
      {isTestMode && (
        <div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-4">
          ...
          <p className="text-xs text-amber-600 mb-3">Skip payment to test the application flow without real charges.</p>
          <Button
            variant="outline"
            size="sm"
            disabled={loading}
            onClick={async () => {
              ...
              try {
                ...
                await completeApplication("TEST-MOCK-" + Date.now(), "TEST")
