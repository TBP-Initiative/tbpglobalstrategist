...
async function processPaymentComplete(userId: string, orderId: string, amount: string) {
  ...
  if (referral) {
    await prisma.referral.update({ where: { id: referral.id }, data: { status: "WAITING_APPROVAL" } })
    ...
    if (!credit) {
      ...
      await prisma.notification.create({
        data: {
          ...
          message: "Someone you referred has completed payment. Your £50 reward is in a 30-day hold period before it becomes available.",
          type: "SYSTEM",
        },
      })
    }
...
export async function POST(req: Request) {
  try {
    ...
    console.error("POST /api/payment/capture error:", err)
